...
app.get('/travelerSignInScreen', (req, res) => {
    res.render('travelerSignInScreen', {wrongCredentials});
});
...
app.get('/selectTheTripD', redirectLogin, async (req, res) => {
...
app.post('/hotel', redirectAgentLogin, async (req, res) => {
    ...
    if (id.length == 0 && name.length == 0) {
        ...
        console.log(`select * from hotel where hotelname = '${name}'`);
...
app.post('/resturaunt', redirectAgentLogin, async (req, res) => {
    ...
    if (id.length == 0 && name.length == 0) {
        ...
        console.log(`select * from hotel where hotelname = '${name}'`);
        let [result, fields] = await db.execute(`select * from resturaunt where resturauntname = '${name}'`);
        resturaunts = result;
    }
    show = false;
